...
class ColorBarVis {
    ...
    wrangleData () {
        ...
        let list = Array.from(new Set(vis.data.map(d=>d.color)));

        // add all countries to selection list
        list.push("all colors");

        // sort selection list so that all colors is at the top
        // and color are in alphabetical order
        vis.colorList = list.sort((a, b) => {
            return a.localeCompare(b, 'en', { sensitivity: 'base' });
        });
        // console.log(vis.colorList)

        // create counts by color
        vis.colorCounts = [];

        vis.colorList.forEach((entry) => {
            if (entry !== "") {
                let counts = 0;
    ...
    updateVis() {
        ...
        vis.x.domain([0, d3.max(vis.colorCounts, d=>+d.count)]);

        vis.text = vis.svg.selectAll(".color-text").data(vis.colorCounts, d=>d.color);
        ...
        vis.text.enter()
            ...
            .on("click", function (event) {
                ...
                if (event.target.innerHTML === selectedColor) {
                    d3.select(this)
                        .style("fill", "lightgray")
                        .style("font-size", "0.8vw")
                        .style("font-weigth", "normal");
        ...
        vis.rect = vis.svg.selectAll(".color-rect").data(vis.colorCounts, d => d.color);
